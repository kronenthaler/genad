		...
		<ul id="relation" style="padding-right: 30px;">
			<xsl:for-each select="/entity/relations/relation">
				<xsl:if test="@active = 'selected'">
					<li><a class="{@active}" href="{/entity/prefix}list{@class}.{//@ext}?rel_id={//@id}&amp;class={@class}&amp;currentClass={@currentClass}&amp;{$ids}"><span><h3><xsl:value-of select="."/>:&nbsp;<xsl:call-template name="msg-listing"/></h3></span></a></li>
				</xsl:if>	
				<xsl:if test="@active != 'selected'">
					<li><a class="{@active}" href="{/entity/prefix}list{@class}.{//@ext}?rel_id={//@id}&amp;class={@class}&amp;currentClass={@currentClass}&amp;{$ids}"><span><h3><xsl:value-of select="."/></h3></span></a></li>
				</xsl:if>
			</xsl:for-each>
			<li><a href="{/entity/prefix}mod{//@currentClass}.{//@ext}?action=mod&amp;id={//@id}&amp;{$ids}"><span><h3><xsl:call-template name="msg-edit"/>&nbsp;<xsl:value-of select="/entity/title"/></h3></span></a></li>
		</ul>
		<div id="content">
			<center>
															...
															<td class="part2">
																<xsl:choose>
																	<xsl:when test="@type = 'radio' or @type = 'checkbox'">
																		<center><a href="{/entity/prefix}exec{//@name}.{//@ext}?currentClass={//@currentClass}&amp;rel_id={//@id}&amp;ini={/entity/pager/offset}&amp;action=mod&amp;id={$key}&amp;str_{@map}={1 - $value}{$ids}"><img src="images/{$value}.gif"/></a></center>
																	</xsl:when>
																	<xsl:when test="@type = 'image'">
	...
	<xsl:template match="pager">
		...
		<td colspan="2" align="right">
			<xsl:if test="offset > 0">
				<a href="{/entity/prefix}list{//@name}.{//@ext}?currentClass={//@currentClass}&amp;rel_id={//@id}&amp;ini={offset - page-size}{$ids}"><img src="images/less.gif" class="border" align="absmiddle"/></a>&nbsp;
			</xsl:if>
			...
			<xsl:if test="offset &lt; total-rows - page-size">
				&nbsp;<a href="{/entity/prefix}list{//@name}.{//@ext}?currentClass={//@currentClass}&amp;rel_id={//@id}&amp;ini={offset + page-size}{$ids}"><img src="images/more.gif" class="border" align="absmiddle"/></a>
			</xsl:if>
		</td>
	</xsl:template>
</xsl:stylesheet>
